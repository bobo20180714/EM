...
var inShowNode;
var orgEmployeeCheckTreeSetting = {//树形配置;
	check : {
		enable : true,
		chkboxType : {
			"Y" : "ps",
...
//初始化左边树形菜单;
function loadOrgEmployeeCheckTreeWithParam(url,param){
	...
	$.ajax({
		...
		success: function(data) {
			...
			$.fn.zTree.init($("#zTree"), orgEmployeeCheckTreeSetting, zNodes);
			funcZTree = $.fn.zTree.getZTreeObj("zTree");
		}
	});
}
function loadOrgEmployeeCheckTree(url){
	$.ajax({
		type: "post",
		url: url,
		dataType:"json",
