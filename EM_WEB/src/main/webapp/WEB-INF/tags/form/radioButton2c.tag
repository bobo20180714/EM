...
<c:if test="${not empty typeCode}">
	<c:forEach items="${fns:getDictList(typeCode)}" var='dict'>
		<input class="${cls}" type="radio" name="${name}" <c:if test="${not empty disabled}">disabled="disabled"</c:if> value="${dict.VALUE}" ${defaultValue==dict.value?'checked="checked"':''} />${dict.TEXT}
	</c:forEach>
</c:if>
