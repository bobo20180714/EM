...
public class StringUtils extends org.apache.commons.lang.StringUtils {
	...
	public static boolean getBoolean(Object str) {
		...
		if (str == null) {
			...
			return str.toString().trim().equals("true");
		}
	}
	...
	public static String replaceSpeChar(String replaceText) {
		if (null != replaceText && !"".equals(replaceText)) {
			String[] speCharArray = { "<br />", "&micro;", "</f14>", "<f14>", "&Phi;", "&nbsp;", "&amp;", "&times;", "&mu;", "&quot;", "&plusmn;", "&alpha;", "&phi;", "&ldquo;", "&rdquo;", "&trade;",
					"&reg;", "&divide;", "<div>", "</div>", "<DIV>", "</DIV>" };
			...
			for (int i = 0; i < speCharArray.length; i++) {
				while (replaceText.indexOf(speCharArray[i]) != -1) {
					...
					replaceText = replaceText.substring(0, j) + parCharArray[i] + replaceText.substring(j + speCharArray[i].length());
				}
			}
		}
		return replaceText;
	...
	public static void main(String[] args) {
		...
		// String s2 =
		// "ysG3MHPZdLx5vO1N0TeRtcmljkziK4bECWoIpuJ62AVrBUDnX9SgFYq7wfh8aQ";
		// Map<Character, Character> charMap = new HashMap<Character,
		// Character>(s1.length());
		...
		/*
		 * System.out.println("36位："+uniqueKey36()); String specialStr =
		 * "&divide;"; String str1 = HtmlUtils.htmlEscape(specialStr);
		 * //①转换为HTML转义字符表示 System.out.println(str1);
		 * 
