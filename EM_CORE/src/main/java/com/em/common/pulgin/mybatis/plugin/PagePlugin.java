package com.em.common.pulgin.mybatis.plugin;
...
import com.em.common.page.WebTool;
import com.em.common.pulgin.jdbc.dialet.Dialect;
...
public class PagePlugin implements Interceptor {
	...
	public Object intercept(Invocation ivk) throws Throwable {
		if (ivk.getTarget() instanceof RoutingStatementHandler) {
			 ...
			 */
			if (mappedStatement.getId().toLowerCase().matches(pageSqlId.toLowerCase())) { // 拦截需要分页的SQL
				...
				if (parameterObject == null) {
					...
					ResultSet rs = null;
					if (pageView.isAutoCount()) {
						try {
							Connection connection = (Connection) ivk.getArgs()[0];
							String countSql = "select count(1) from (" + sql.replaceAll("\\bSYS_DIC\\b", "SYS_DIC_EMPTY") + ") tmp_count"; // 记录统计
							if (mappedStatement.getId().toLowerCase().indexOf("sysdictmapper") != -1) {
					...
					try {
						String pageSql = "";
						pageSql = generatePagesSql(sql, pageView);
	...
	private String generatePagesSql(String sql, PageView page) {
		if (page != null && dialectObject != null) {
			...
			// sql中没有order by 且 有order by参数 且 是mysql的请求才会启用排序，默认排序请在前台js中设置
			if (sql.indexOf("ORDER BY") == -1 && sql.indexOf("order by") == -1 && !CollectionUtils.isEmpty(page.getOrder())
					&& (dialect.indexOf("MySQLDialect") != -1 || dialect.indexOf("SQLServer2008Dialect") != -1 || dialect.indexOf("OracleDialect") != -1)) {
				// 增加排序和分页sql
				return dialectObject.getLikeOrderLimitString(sql, page);
			}

			// pageNow默认是从1，而已数据库是从0开始计算的．所以(page.getPageNow()-1)
			return dialectObject.getLimitString(sql, page.getStart(), page.getLength());
		}
		return sql;
	}
