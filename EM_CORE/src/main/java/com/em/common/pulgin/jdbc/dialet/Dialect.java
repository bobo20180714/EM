...
public class Dialect {
	...
	public String getLikeOrderLimitString(String sql, PageView page) {
		int offset = page.getStart();
		int limit = page.getLength();
		List<OrderBy> orders = page.getOrder();
		List<Columns> searchAbleCols = page.getSearchAbleCols();
		String likeStr = page.getSearch().getValue();

		return getLikeOrderLimitString(sql, offset, Integer.toString(offset), limit, Integer.toString(limit), orders, searchAbleCols, likeStr);
	}
	...
	public String getLikeOrderLimitString(String sql, int offset, String offsetPlaceholder, int limit, String limitPlaceholder, List<OrderBy> orders, List<Columns> searchAbleCols, String likeStr) {
		throw new UnsupportedOperationException("getOrderLimit queries not supported");
	}
}
