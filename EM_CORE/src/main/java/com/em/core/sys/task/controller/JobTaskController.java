package com.em.core.sys.task.controller;
...
import com.em.core.sys.task.service.JobTaskService;

@Controller
@RequestMapping("/sysJobTask")
public class JobTaskController {
	...
	@ResponseBody
	public PageView getPageData(HttpServletRequest request) {
		PageView<ScheduleJob> pageView = new PageView<ScheduleJob>();
		List<ScheduleJob> taskList = taskService.getAllTask();
		pageView.setData(taskList);
		return pageView;
	}

	@RequestMapping("forQuery")
	public String taskList(HttpServletRequest request) {
		List<ScheduleJob> taskList = taskService.getAllTask();
		request.setAttribute("taskList", taskList);
		return "core/sys/task/task_forQuery";
	}
	...
	public RetObj taskList(HttpServletRequest request, ScheduleJob scheduleJob) {
		...
		try {
			if (StringUtils.isNotBlank(scheduleJob.getSpringId())) {
				obj = ComponentFactory.getBean(scheduleJob.getSpringId());
			} else {
	...
	public RetObj batchDelete(HttpServletRequest request, String ids) {
		...
		try {
			...
			taskService.batchDelete(ids.split(","));
